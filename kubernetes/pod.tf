resource "signalfx_detector" "k8s_pod_status" {
  ...
  program_text = <<-EOF
    ...
    against_recent.detector_mean_std(stream=A, current_window='5m', historical_window='30m', fire_num_stddev=2, clear_num_stddev=2, orientation='above', ignore_extremes=True, calculation_mode='vanilla').publish('K8S Pod Status Waiting Detector') 
  EOF
  rule {
...
resource "signalfx_detector" "k8s_pods_active" {
  ...
  program_text = <<-EOF
    ...
    against_recent.detector_mean_std(stream=A, current_window='10m', historical_window='1h', fire_num_stddev=3.5, clear_num_stddev=3, orientation='out_of_band', ignore_extremes=True, calculation_mode='vanilla').publish('K8S Active Pods Detector')
  EOF
  rule {
